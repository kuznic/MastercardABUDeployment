...
declare @SQL varchar(max) = N'CREATE PROCEDURE [dbo].[get_all_chained_cards_encrypted] @bins VARCHAR(200), @page_num INT,
@page_size INT AS
SELECT
COUNT(*) AS count
FROM
...
LEFT JOIN
  (
      SELECT
          pc_cards_abu.pan_encrypted old_pan,
          ...
          pc_card_accounts.account_id old_account_id
      FROM
          pc_cards_abu with (nolock) INNER JOIN pc_card_accounts
  ON
      pc_cards_abu.pan = pc_card_accounts.pan
  ...
  old_cards
ON new_cards.account_id = old_cards.old_account_id
